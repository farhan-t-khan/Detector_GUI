...
#define BS_TYPEMASK_EX SS_TYPEMASK
...
CButtonST::CButtonST()
{
   ...
   //when a flat border is wanted without 3d look
   m_bDrawFlatBorder = FALSE;
	...
	m_nTypeStyle = BS_TYPEMASK_EX;

	// By default the button is not a checkbox
	m_bIsCheckBox = FALSE;
	m_nCheck = 0;

   ::ZeroMemory(m_crColors, sizeof(m_crColors));
   ::ZeroMemory(m_crColorsSystem, sizeof(m_crColors));
	// Set default colors
...
void CButtonST::PreSubclassWindow() 
{
	...
	m_nTypeStyle = nBS & BS_TYPEMASK_EX;
	...
	ModifyStyle(BS_TYPEMASK_EX, BS_OWNERDRAW, SWP_FRAMECHANGED);
...
{
	UINT_PTR nNewType = (wParam & BS_TYPEMASK_EX);
	...
	return DefWindowProc(BM_SETSTYLE,
		(wParam & ~BS_TYPEMASK_EX) | BS_OWNERDRAW, lParam);
} // End of OnSetStyle
...
DWORD CButtonST::SetIcon(HICON hIconIn, HICON hIconOut)
{
	...
	if (hIconIn)
	{
		...
		if (hIconOut)
		{
         UINT_PTR nIconOut = reinterpret_cast<UINT_PTR>(hIconOut);
			switch (nIconOut)
			{
				case BTNST_AUTO_GRAY:
...
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	...
	if (hBitmapIn)
	{
      ...
      UINT_PTR nBitmapOut = reinterpret_cast<UINT_PTR>(hBitmapOut);
		switch (nBitmapOut)
		{
			case BTNST_AUTO_GRAY:
...
DWORD CButtonST::SetFlatBorderAlways(BOOL bBoarder, BOOL bRepaint)
{
   m_bDrawFlatBorder = bBoarder;
...
{
   if (m_crColorsSystem[BTNST_COLOR_BK_IN] == m_crColors[BTNST_COLOR_BK_IN])
      m_crColorsSystem[BTNST_COLOR_BK_IN] = m_crColors[BTNST_COLOR_BK_IN] = ::GetSysColor(COLOR_BTNFACE);
   if (m_crColorsSystem[BTNST_COLOR_FG_IN] == m_crColors[BTNST_COLOR_FG_IN])
      m_crColorsSystem[BTNST_COLOR_FG_IN] = m_crColors[BTNST_COLOR_FG_IN] = ::GetSysColor(COLOR_BTNTEXT);
   if (m_crColorsSystem[BTNST_COLOR_BK_OUT] == m_crColors[BTNST_COLOR_BK_OUT])
      m_crColorsSystem[BTNST_COLOR_BK_OUT] = m_crColors[BTNST_COLOR_BK_OUT] = ::GetSysColor(COLOR_BTNFACE);
   if (m_crColorsSystem[BTNST_COLOR_FG_OUT] == m_crColors[BTNST_COLOR_FG_OUT])
      m_crColorsSystem[BTNST_COLOR_FG_OUT] = m_crColors[BTNST_COLOR_FG_OUT] = ::GetSysColor(COLOR_BTNTEXT);
   if (m_crColorsSystem[BTNST_COLOR_BK_FOCUS] == m_crColors[BTNST_COLOR_BK_FOCUS])
      m_crColorsSystem[BTNST_COLOR_BK_FOCUS] = m_crColors[BTNST_COLOR_BK_FOCUS] = ::GetSysColor(COLOR_BTNFACE);
   if (m_crColorsSystem[BTNST_COLOR_FG_FOCUS] == m_crColors[BTNST_COLOR_FG_FOCUS])
      m_crColorsSystem[BTNST_COLOR_FG_FOCUS] = m_crColors[BTNST_COLOR_FG_FOCUS] = ::GetSysColor(COLOR_BTNTEXT);
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	...
	if (m_bIsFlat == FALSE || m_bDrawFlatBorder)
	{
		if (m_bIsFocused || m_bIsDefault || m_bDrawFlatBorder)
		{
			CBrush br(RGB(0,0,0));  
