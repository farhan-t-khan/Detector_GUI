...
*/
void NMEAParser::ProcessGeneral(const CHAR *buf, const UINT bufSize)
{
   ...
   if (bufSize < 6)
      return;
   strncpy(auxBuf, buf, 6);
   auxBuf[6] = '\0';
   if (strcmp(auxBuf, "$GNGGA") != 0 || buf[6] != ',')
      return;
   p1 += 7;
...
void NMEAParser::ProcessGPGGA(const CHAR *buf, const UINT bufSize)
{
	...
	strncpy(auxBuf, buf, 6);
	auxBuf[6] = '\0';
	if(strcmp(auxBuf, "$GPGGA") != 0 || buf[6] != ',')
		return;
	p1 += 7;
